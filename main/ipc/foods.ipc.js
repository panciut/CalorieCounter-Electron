...
function registerFoodsIpc() {
  ...
  ipcMain.handle('foods:add', (_, { name, calories, protein, carbs, fat, fiber, piece_grams, is_liquid, is_bulk, barcode, opened_days, discard_threshold_pct, price_per_100g, image_url }) => {
    const bulk = is_bulk ? 1 : 0;
    const piece = bulk ? null : (piece_grams || null);
    const result = getDb().prepare(
      'INSERT INTO foods (name, calories, protein, carbs, fat, fiber, piece_grams, is_liquid, is_bulk, barcode, opened_days, discard_threshold_pct, price_per_100g, image_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    ).run(name, calories, protein || 0, carbs || 0, fat || 0, fiber || 0, piece, is_liquid ? 1 : 0, bulk, barcode || null, opened_days ?? null, discard_threshold_pct ?? 5, price_per_100g ?? null, image_url || null);
  ...
  ipcMain.handle('foods:update', (_, { id, name, calories, protein, carbs, fat, fiber, piece_grams, is_liquid, is_bulk, barcode, opened_days, discard_threshold_pct, price_per_100g, image_url }) => {
    const bulk = is_bulk ? 1 : 0;
    const piece = bulk ? null : (piece_grams || null);
    getDb().prepare(
      'UPDATE foods SET name=?, calories=?, protein=?, carbs=?, fat=?, fiber=?, piece_grams=?, is_liquid=?, is_bulk=?, barcode=?, opened_days=?, discard_threshold_pct=?, price_per_100g=?, image_url=? WHERE id=?'
    ).run(name, calories, protein || 0, carbs || 0, fat || 0, fiber || 0, piece, is_liquid ? 1 : 0, bulk, barcode || null, opened_days ?? null, discard_threshold_pct ?? 5, price_per_100g ?? null, image_url || null, id);
    return { ok: true };
  });
